...
The baseline verified by [`docs/todo/README_todo.md`](../../docs/todo/README_todo.md) now includes completed Phases 00 and 01 plus the first six validated Phase 02 sessions while remaining deliberately smaller than the full production-hardening scope:
...
- Closed production-eval/result contracts and a frozen 18-case synthetic set
  ...
  failure. Deliberate Session 07 break/revert traces remain pending.
...
- Biome formatting and linting, strict TypeScript, 269 deterministic tests, and all 18 durable production-eval cases pass through `npm run verify`.
...
The todo index and tasks `00` through `08` are the authoritative ordered delivery plan. All nine tasks are required. Tasks `00` through `04` are complete; tasks `05` through `08` remain planned in their later phases. Tasks `00` through `07` occupy Weeks 1 through 4, and task `08` occupies Week 5 after its measurement entry gate passes. Completing the task `08` experiment and its keep-or-remove decision is mandatory even when the evidence says to remove the added handoff. Every behavior in a task's `Work` section remains planned until its acceptance evidence exists; this PRD must not imply otherwise.
...
| 02 | 3 | Recovery and Evaluation Gates | `04`, `05` | 7 | In Progress (6/7) |
...
- Project state registers completed Phases 00 and 01 and current in-progress
  Phase 02 at 6/7 sessions: Phases 03 and 04 remain PRD-planned future phases
...
- The root README calls the repository a runnable reference while the task
  ...
  tasks `00`-`04` as verified, Phase 02 Sessions 01-06 as complete Task `04`
  evidence plus the Task `05` definition and executable-gate milestones, and keep tasks `05`-`08`
  incomplete until their full acceptance
  evidence exists.
